...
package com.llamalad7.hyperiumfriendsgui.gui;
...
public class HypixelFriendsGui extends HyperiumGui {
    ...
    protected void pack() {
        int textWidth = Math.max(new ScaledResolution(Minecraft.getMinecraft()).getScaledWidth() / 9, 100);
        int height = 20;
        if (textField == null)
            textField = new GuiTextField(nextId(), Minecraft.getMinecraft().fontRendererObj, new ScaledResolution(Minecraft.getMinecraft()).getScaledWidth() / 2 - textWidth / 2, 25, textWidth, height);

        reg("SORT", new GuiButton(nextId(), new ScaledResolution(Minecraft.getMinecraft()).getScaledWidth() - 153, 23, 150, 20, "Sort by: "), guiButton -> {
        ...
        reg("PARTY", new GuiButton(nextId(), new ScaledResolution(Minecraft.getMinecraft()).getScaledWidth() - 153, 23 + 21, 150, 20, "Party Selected"), guiButton -> {
            Iterator<HypixelApiFriendObject> iterator = selected.iterator();
            while (iterator.hasNext()) {
                HypixelApiFriendObject next = iterator.next();
                if (iterator.hasNext())
                    Minecraft.getMinecraft().thePlayer.sendChatMessage("/party invite " + next.getName());
                else {
                    Minecraft.getMinecraft().thePlayer.sendChatMessage("/party invite " + next.getName());
        ...
        reg("REMOVE", new GuiButton(nextId(), new ScaledResolution(Minecraft.getMinecraft()).getScaledWidth() - 153, 23 + 21 * 2, 150, 20, "Remove (Hold down)"), guiButton -> {
            ...
            if (guiButton.isMouseOver() && Mouse.isButtonDown(0) && guiButton.enabled) {
                ...
                if (removeTicks >= totalTick) {
                    ...
                    while (iterator.hasNext()) {
                        HypixelApiFriendObject next = iterator.next();
                        if (iterator.hasNext())
                            Minecraft.getMinecraft().thePlayer.sendChatMessage("/friend remove " + next.getName());
                        else {
                            Minecraft.getMinecraft().thePlayer.sendChatMessage("/friend remove " + next.getName());
                            guiButton.enabled = true;
                        }
        ...
        for (GuiBoxItem<HypixelApiFriendObject> selectedBox : friendListBoxes) {
            if (selectedBox.getBox().isMouseOver(mouseX, mouseY)) {
                if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
                    ...
                    Minecraft.getMinecraft().thePlayer.sendChatMessage("/party invite " + selectedBox.getObject().getName());
    ...
    private void rebuildFriends() {
        try {
            //HypixelAPI instance = new HypixelAPI();
            CompletableFuture<HypixelApiFriends> userfriends = instance.getFriendsForCurrentUser();
            HypixelApiFriends thingy = userfriends.get();
            JsonHolder thing = thingy.getData();
            HypixelFriends thing2 = new HypixelFriends(thing);
            this.friends = thing2;
        } catch (InterruptedException | ExecutionException e) {
            e.printStackTrace();
        }
        friends.sort(sortType);
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        //Some long name
        final int bottomRenderBound = new ScaledResolution(Minecraft.getMinecraft()).getScaledHeight() / 9 * 8;
        ...
        GuiBlock friendsBlock = new GuiBlock(namesBlock.getRight() + 15, new ScaledResolution(Minecraft.getMinecraft()).getScaledWidth() - 100, topRenderBound, bottomRenderBound);
