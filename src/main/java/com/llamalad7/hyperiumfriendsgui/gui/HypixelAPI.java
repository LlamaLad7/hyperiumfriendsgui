...
//import cc.hyperium.event.SubscribeEvent;
...
public class HypixelAPI {
    ...
    public static Sk1erMod sk1erMod;
    ...
    public UUID dashMeUp(String uuidWithoutDashes) {
        ...
        return getFriends(getKeyForCurrentUser()).whenComplete((data, error) -> {
            ...
            data.getFriends().forEach(
                friend -> friendsForCurrentUser.add(
                    dashMeUp(new JsonHolder(friend.getAsJsonObject()).optString("uuid"))
                )
            );
        });
    }
